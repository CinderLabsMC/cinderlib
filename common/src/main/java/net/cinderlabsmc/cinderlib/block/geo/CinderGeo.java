...
/**
 * GeckoLib settings of a block. Contains no GeckoLib types, so it is safe
 * without GeckoLib at runtime.
 *
 * <p>
 * Default asset paths for block {@code mod:name}:
 * <ul>
 * <li>model: {@code assets/mod/geckolib/models/block/name.geo.json} (also used
 * for the item unless {@link #itemModel(Identifier)} is set)</li>
 * <li>animation:
 * {@code assets/mod/geckolib/animations/block/name.animation.json} (only needed
 * with controllers)</li>
...
public final class CinderGeo {
  ...
  /**
   * Model id, e.g. {@code mod:block/rack_shelf} for
   * {@code geckolib/models/block/rack_shelf.geo.json}.
   */
  ...
  /**
   * Animation file id, e.g. {@code mod:block/rack_shelf} for
   * {@code geckolib/animations/block/rack_shelf.animation.json}.
   */
  ...
  /**
   * Loops this animation (by name inside the animation file) without writing a
   * controller.
   */
