 ...
 * }
 * }</pre>
 * Call {@link #register()} once from common init (Fabric {@code ModInitializer} / NeoForge mod constructor).
 */
public final class CinderRegistrar {
    ...
    /** A block entity type that several blocks can share via {@link CinderBlockBuilder#blockEntity(CinderBlockEntityType)}. */
    public <E extends CinderBlockEntity> @NonNull CinderBlockEntityType<E> blockEntity(@NonNull String name, CinderBlockEntity.@NonNull Factory<E> factory) {
        checkOpen(name);
        return new CinderBlockEntityType<>(this, id(name), factory);
    }

    /** Initializes the given classes so their static {@link CinderBlockType} fields are created before {@link #register()}. */
    ...
    /** Registers blocks, then items, then block entity types, and on the client the GeckoLib renderers. */
