...
public record FlatTheme(Palette palette) implements ICinderTheme {
  ...
  public void panel(@NonNull GuiGraphicsExtractor graphics, @NonNull Bounds bounds) {
    ...
    bevel(graphics, new Bounds(left + LINE, top + LINE, bounds.width() - 2 * LINE, bounds.height() - 2 * LINE),
        palette.panelLight(), palette.panelDark(), palette.panel());
  }

  @Override
  public void slot(@NonNull GuiGraphicsExtractor graphics, int itemLeft, int itemTop) {
    inset(graphics, new Bounds(itemLeft - GuiSizes.SLOT_BORDER, itemTop - GuiSizes.SLOT_BORDER, GuiSizes.SLOT_SIZE,
        GuiSizes.SLOT_SIZE));
  }
  ...
  public void button(@NonNull GuiGraphicsExtractor graphics, @NonNull Bounds bounds, boolean hovered, boolean active) {
    ...
    var inner = new Bounds(bounds.left() + LINE, bounds.top() + LINE, bounds.width() - 2 * LINE,
        bounds.height() - 2 * LINE);
