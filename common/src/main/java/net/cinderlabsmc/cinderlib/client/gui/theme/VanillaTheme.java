...
public final class VanillaTheme implements ICinderTheme {
    ...
    private static final Identifier HIGHLIGHT_BACK_SPRITE = Identifier.withDefaultNamespace("container/slot_highlight_back");
    private static final Identifier HIGHLIGHT_FRONT_SPRITE = Identifier.withDefaultNamespace("container/slot_highlight_front");
    ...
    private static final WidgetSprites BUTTON_SPRITES = new WidgetSprites(Identifier.withDefaultNamespace("widget/button"), Identifier.withDefaultNamespace("widget/button_disabled"), Identifier.withDefaultNamespace("widget/button_highlighted"));
    ...
    public void panel(@NonNull GuiGraphicsExtractor graphics, @NonNull Bounds bounds) {
        ...
        panelPart(graphics, new Bounds(left + CORNER, top + CORNER, innerWidth, innerHeight), CORNER, CORNER, CENTER, CENTER);
    }

    @Override
    public void slot(@NonNull GuiGraphicsExtractor graphics, int itemLeft, int itemTop) {
        graphics.blitSprite(RenderPipelines.GUI_TEXTURED, SLOT_SPRITE, itemLeft - GuiSizes.SLOT_BORDER, itemTop - GuiSizes.SLOT_BORDER, GuiSizes.SLOT_SIZE, GuiSizes.SLOT_SIZE);
    }
    ...
    public void button(@NonNull GuiGraphicsExtractor graphics, @NonNull Bounds bounds, boolean hovered, boolean active) {
        graphics.blitSprite(RenderPipelines.GUI_TEXTURED, BUTTON_SPRITES.get(active, hovered), bounds.left(), bounds.top(), bounds.width(), bounds.height());
    }
    ...
    public void scrollHandle(@NonNull GuiGraphicsExtractor graphics, @NonNull Bounds bounds, boolean active) {
        ...
        graphics.blitSprite(RenderPipelines.GUI_TEXTURED, sprite, bounds.left(), bounds.top(), bounds.width(), bounds.height());
    }

    @Override
    public void highlightBack(@NonNull GuiGraphicsExtractor graphics, int itemLeft, int itemTop) {
        graphics.blitSprite(RenderPipelines.GUI_TEXTURED, HIGHLIGHT_BACK_SPRITE, itemLeft - HIGHLIGHT_OFFSET, itemTop - HIGHLIGHT_OFFSET, HIGHLIGHT_SIZE, HIGHLIGHT_SIZE);
    }

    @Override
    public void highlightFront(@NonNull GuiGraphicsExtractor graphics, int itemLeft, int itemTop) {
        graphics.blitSprite(RenderPipelines.GUI_TEXTURED, HIGHLIGHT_FRONT_SPRITE, itemLeft - HIGHLIGHT_OFFSET, itemTop - HIGHLIGHT_OFFSET, HIGHLIGHT_SIZE, HIGHLIGHT_SIZE);
    }
    ...
    private static void panelPart(GuiGraphicsExtractor graphics, Bounds target, int sourceLeft, int sourceTop, int sourceWidth, int sourceHeight) {
        if (target.width() <= 0 || target.height() <= 0) {
            return;
        }

        graphics.blit(RenderPipelines.GUI_TEXTURED, PANEL_TEXTURE, target.left(), target.top(), sourceLeft, sourceTop, target.width(), target.height(), sourceWidth, sourceHeight, TEXTURE_SIZE, TEXTURE_SIZE);
    }
}
